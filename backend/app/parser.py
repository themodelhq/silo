...
FIELD_PATTERNS: dict[str, str] = {
    ...
    "basic_salary": rf"(?:basic|base)\s*(?:salary|pay|allowance)?\s*[:\-=]?\s*{_NUM}",
...
# name, even if a dedicated structured field (basic_salary, etc.) also
...
_LINE_ITEM_EXCLUDE_STATUTORY_KEYWORDS = ("tax", "paye", "pension", "ssnit", "nssf", "uif", "nhf")
...
def _extract_line_items(text: str) -> list[dict]:
    ...
    for line in text.splitlines():
        ...
        if any(keyword in normalized for keyword in _LINE_ITEM_EXCLUDE_KEYWORDS):
            continue
        is_earnings_style = "allowance" in normalized or "income" in normalized
        if not is_earnings_style and any(keyword in normalized for keyword in _LINE_ITEM_EXCLUDE_STATUTORY_KEYWORDS):
            continue
