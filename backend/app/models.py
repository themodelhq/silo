...
class PayoutTransfer(Base):
    """An outbound Paystack transfer — money leaving an envelope to an
    arbitrary bank account the user specifies (the "Account" option in the
    envelope Transfer To dropdown). Distinct from PaymentAccount (money
    *received* into a dedicated account) — this is money *sent out*.

    Like the DVA flow, this is asynchronous: initiate_transfer() only
    confirms Paystack accepted the request ("pending", or "otp" if the
    Paystack business hasn't disabled transfer OTP confirmation). The
    envelope balance is debited optimistically on creation and refunded if
    the webhook later reports transfer.failed / transfer.reversed.
    """
    ...
    provider = Column(String, default="paystack")
    provider_reference = Column(String, unique=True, nullable=False)
    provider_transfer_code = Column(String, nullable=True)
    provider_recipient_code = Column(String, nullable=True)
    recipient_account_number = Column(String, nullable=False)
    recipient_account_name = Column(String, nullable=True)
    recipient_bank_code = Column(String, nullable=False)
    recipient_bank_name = Column(String, nullable=True)
    amount = Column(Float, nullable=False)
    reason = Column(String, nullable=True)
    currency = Column(String, default="NGN")
    status = Column(String, default="pending")  # pending | otp_required | success | failed
    failure_reason = Column(String, nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow)
