...
def _current_secret_key() -> str:
    """Prefer a live environment lookup over the value captured at import
    time — if the platform's env var was set/changed without the process
    fully restarting, this still picks it up. Falls back to the
    module-level PAYSTACK_SECRET_KEY (which tests monkeypatch directly)."""
    return os.environ.get("PAYSTACK_SECRET_KEY") or PAYSTACK_SECRET_KEY


def _require_secret_key() -> str:
    key = _current_secret_key()
    if not key:
...
def default_preferred_bank() -> str:
    """Paystack requires "test-bank" as the preferred_bank when creating a
    dedicated virtual account with a *test* secret key (sk_test_...) — the
    live-mode bank slugs ("wema-bank", "titan-paystack") are silently
    rejected with a test key, and vice versa. Since testing before going
    live is the normal, recommended workflow, auto-detecting this avoids a
    confusing rejection that looks identical to a missing/invalid key.
    Set PAYSTACK_PREFERRED_BANK explicitly to override this detection."""
    configured = os.getenv("PAYSTACK_PREFERRED_BANK")
    if configured:
        return configured
    return "test-bank" if _current_secret_key().startswith("sk_test_") else "wema-bank"
...
def create_dedicated_virtual_account(
    customer_code: str, preferred_bank: Optional[str] = None
) -> dict:
    ...
    preferred_bank = preferred_bank or default_preferred_bank()
    try:
        resp = httpx.post(
