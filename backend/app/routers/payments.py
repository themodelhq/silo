...
def get_banks(current_user: models.User = Depends(auth.get_current_user)):
    """Powers the bank picker for the "Account" transfer option — this list
    rarely changes, so the frontend should cache it for the session rather
    than calling this on every modal open."""
    try:
        return payments.list_banks()
    except payments.PaymentProviderError as exc:
        raise HTTPException(status_code=502, detail=str(exc))


@router.post("/resolve-account", response_model=schemas.ResolveAccountOut)
def resolve_account(
    payload: schemas.ResolveAccountRequest,
    current_user: models.User = Depends(auth.get_current_user),
):
    """Confirms a destination account is real and returns the name on it, so
    the person can double-check before sending money — call this before
    POST /payments/transfers, the same way every Nigerian banking app shows
    a "confirm recipient" step."""
    try:
        result = payments.resolve_account(payload.account_number, payload.bank_code)
    except payments.PaymentProviderError as exc:
        raise HTTPException(status_code=502, detail=str(exc))
    return schemas.ResolveAccountOut(**result)


@router.post("/transfers", response_model=schemas.PayoutTransferOut, status_code=201)
def create_transfer(
    payload: schemas.TransferRequest,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(auth.get_current_user),
):
    """Send money out of an envelope to any bank account — the "Account"
    option in the envelope Transfer To dropdown. The envelope is debited
    immediately (optimistic — matches how a real bank app shows a pending
    debit right away); if the transfer later fails, the webhook handler
    refunds it. See models.PayoutTransfer for the full lifecycle."""
    envelope = (
        db.query(models.Envelope)
        .filter(models.Envelope.id == payload.envelope_id, models.Envelope.user_id == current_user.id)
        .first()
    )
    if not envelope:
        raise HTTPException(status_code=404, detail="Envelope not found.")
    if envelope.locked:
        raise HTTPException(status_code=400, detail=f"{envelope.name} is locked — unlock it before transferring out of it.")
    if envelope.balance < payload.amount:
        raise HTTPException(status_code=400, detail=f"Insufficient balance in {envelope.name}.")

    reference = models.gen_uuid()
    recipient_name = payload.account_name or f"Silo transfer {payload.account_number}"

    try:
        recipient_code = payments.create_transfer_recipient(recipient_name, payload.account_number, payload.bank_code)
        result = payments.initiate_transfer(payload.amount, recipient_code, reference, payload.reason or "")
    except payments.PaymentProviderError as exc:
        raise HTTPException(status_code=502, detail=str(exc))

    bank_name = None
    for bank in _safe_bank_lookup():
        if bank["code"] == payload.bank_code:
            bank_name = bank["name"]
            break

    provider_status = result.get("status")
    status = "otp_required" if provider_status == "otp" else "pending"

    envelope.balance = round(envelope.balance - payload.amount, 2)
    db.add(models.Transaction(
        user_id=current_user.id, envelope_id=envelope.id, type="expense", amount=payload.amount,
        category=envelope.name, merchant=recipient_name, note=f"Transfer to {payload.account_number} (ref {reference})",
    ))

    transfer = models.PayoutTransfer(
        user_id=current_user.id, envelope_id=envelope.id, provider_reference=reference,
        provider_transfer_code=result.get("transfer_code"), provider_recipient_code=recipient_code,
        recipient_account_number=payload.account_number, recipient_account_name=payload.account_name,
        recipient_bank_code=payload.bank_code, recipient_bank_name=bank_name,
        amount=payload.amount, reason=payload.reason, currency=current_user.currency, status=status,
    )
    db.add(transfer)
    db.commit()
    db.refresh(transfer)
...
def _safe_bank_lookup() -> list[dict]:
    try:
        return payments.list_banks()
...
@router.get("/transfers", response_model=list[schemas.PayoutTransferOut])
def list_transfers(
    db: Session = Depends(get_db),
    current_user: models.User = Depends(auth.get_current_user),
):
    return (
        db.query(models.PayoutTransfer)
        .filter(models.PayoutTransfer.user_id == current_user.id)
        .order_by(models.PayoutTransfer.created_at.desc())
        .all()
    )
...
def _handle_transfer_lifecycle_event(db: Session, event: str, data: dict) -> dict:
    """transfer.success / transfer.failed / transfer.reversed — the async
    follow-up to POST /payments/transfers. A failed or reversed transfer
    refunds the envelope it was debited from, so the money doesn't just
    vanish from the user's view."""
    reference = data.get("reference")
    if not reference:
        return {"status": "ignored", "reason": "no_reference_in_payload"}

    transfer = db.query(models.PayoutTransfer).filter(models.PayoutTransfer.provider_reference == reference).first()
    if not transfer:
        logger.warning("Paystack webhook %s: no PayoutTransfer found for reference=%r", event, reference)
        return {"status": "no_matching_transfer"}
    ...
    # transfer.failed or transfer.reversed — refund the envelope it came from.
    transfer.status = "failed"
    transfer.failure_reason = data.get("reason") or data.get("message") or f"Transfer {event.split('.')[1]}."
    if transfer.envelope_id:
        envelope = db.query(models.Envelope).filter(models.Envelope.id == transfer.envelope_id).first()
        if envelope:
            envelope.balance = round(envelope.balance + transfer.amount, 2)
            db.add(models.Transaction(
                user_id=transfer.user_id, envelope_id=envelope.id, type="refund", amount=transfer.amount,
                category=envelope.name, merchant=transfer.recipient_account_name or transfer.recipient_account_number,
                note=f"Refund: transfer {reference} {event.split('.')[1]}",
            ))
    db.commit()
    return {"status": "recorded_failure_and_refunded" if transfer.envelope_id else "recorded_failure"}
...
async def paystack_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    if event in ("transfer.success", "transfer.failed", "transfer.reversed"):
        return _handle_transfer_lifecycle_event(db, event, data)
