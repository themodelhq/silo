...
def _generate_envelope_split_from_payslip(
    ...
    """Turn a payslip's line items into an active EnvelopeRule set and
    matching Envelope records with the *same names* used on the payslip.
    Supersedes whatever split was previously active for this user, so a
    fresh upload cleanly replaces the last one — this is the "specific
    envelope split assigned to that user" the incoming-payment split (see
    routers/payments.py) reads from.
    """
    if not parsed.line_items:
        return []

    # Each rule's percentage is its share of the *full payslip breakdown*
    # (sum of all line items) — not of net salary. Line items are often a
    # gross-level earnings breakdown (basic + every allowance), which
    # doesn't sum to net salary; dividing by net salary there would produce
    # percentages that don't reflect real proportions between envelopes,
    # and — critically — would make split_amount_by_rules over- or
    # under-allocate a real incoming payment relative to what the person
    # actually intended. Dividing by the items' own total instead means
    # percentages always sum to ~100%, so a future payment gets split in
    # exactly the proportions the payslip showed.
    base = sum(li["amount"] for li in parsed.line_items)
    if base <= 0:
        return []
