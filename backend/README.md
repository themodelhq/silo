...
- **Payment API integration** (`app/payments.py`, `routers/payments.py`) —
  `POST /payments/accounts` requests a Paystack dedicated virtual account
  (a real NUBAN account number) for the logged-in user via Paystack's
  single-step `/dedicated_account/assign` API. This is asynchronous on
  Paystack's side: the account is created with `status="pending"` and the
  actual account number/name/bank arrive later through
  `POST /payments/webhook/paystack`, which handles the full lifecycle —
  `customeridentification.success/.failed` (if your Paystack business
  category requires BVN validation) and `dedicatedaccount.assign.success/
  .failed` (the DVA itself) — flipping the record to `"active"` or
  `"failed"` accordingly. Once active, the same webhook handles
  `charge.success` for real inbound payments, splitting the amount across
  that user's *currently active* envelope split — i.e. whatever was
  ...
  redelivery). A phone number is required (Paystack needs it to create the
  account) — pass one in the `POST /payments/accounts` body or set it first
  via `PATCH /auth/me`.
- **Outbound transfers** (`app/payments.py`, `routers/payments.py`) — the
  reverse direction: sending money out of an envelope to any bank account,
  via `GET /payments/banks` (bank picker), `POST /payments/resolve-account`
  (confirms the destination account name before sending — every Nigerian
  banking app does this same "confirm recipient" step), and
  `POST /payments/transfers` (debits the envelope immediately and initiates
  the transfer). Also asynchronous — a transfer starts `"pending"` (or
  `"otp_required"` if the Paystack business hasn't disabled transfer OTP
  confirmation) and the webhook's `transfer.success/.failed/.reversed`
  handling finalizes it, automatically refunding the envelope if it failed.
  This is what powers the "Account" option in the envelope Transfer To
  dropdown in the frontend (see `frontend/js/app.js`).
- **Admin** (`app/auth.py: get_current_admin`, `routers/admin.py`) — a
  ...
  or existing account.
- **REST API** (FastAPI) — auth (register/login/me/update-profile via JWT),
  payslip parsing + persistence, envelope CRUD + lifecycle actions,
  transactions with filtering, and report endpoints (cash flow, expense
  categories, envelope allocation, budget performance). Auto-generated
...
- Live Paystack credentials — the virtual-account and webhook code is real
  and tested against Paystack's documented API/webhook contract, but needs:
  1. a real `PAYSTACK_SECRET_KEY` (without one, `POST /payments/accounts`
     returns a clear `502` rather than failing silently — this is checked
     against the live environment on every call, not just a value cached at
     process startup, so setting/updating it on your host doesn't require a
     code change; whether it takes effect without a restart still depends
     on your platform actually injecting the new value into the running
     process — Render typically restarts the service for you on an env var
     change, but confirm that happened if this still 502s right after
     setting it);
  2. your Paystack business to have completed
     [go-live verification](https://support.paystack.com/hc/en-us/articles/360009881220)
     — dedicated virtual accounts don't work on an unverified account, live
     key or not;
  3. the webhook URL registered by hand in the Paystack Dashboard
     (`https://<your-service>/payments/webhook/paystack`) — Paystack doesn't
     discover it automatically;
  4. **the right `preferred_bank` for your key type** — Paystack requires
     `"test-bank"` when creating a DVA with a *test* secret key (`sk_test_...`)
     and rejects the live-mode slugs (`"wema-bank"`, etc.) with one, and
     vice versa. `app.payments.default_preferred_bank()` auto-detects this
     from your key's prefix, but if you set `PAYSTACK_PREFERRED_BANK`
     explicitly, make sure it actually matches whichever key you're using —
     a mismatch here looks like a generic failure with no obvious cause,
     even though the key itself is completely valid;
  5. if your Paystack business category is **Betting**, **Financial
     Services**, or **General Services**, a BVN + bank account + bank code
     passed to `POST /payments/accounts` (see `schemas.PaymentAccountCreateRequest`)
     — Paystack requires validating the customer's identity before it will
     issue them an account number in those categories. Everything else
     (optional-compliance categories) works with just email/name/phone.
  6. for **outbound transfers** specifically: your Paystack account needs an
     actual funded balance to send from (this moves real money out, unlike
     dedicated virtual accounts which only receive), and, for the automated
     flow in this codebase to complete without manual intervention,
     "Confirm transfers before sending" (transfer OTP) should be turned off
     in Dashboard → Settings → Preferences. If it's left on, transfers come
     back with `status: "otp_required"` instead of completing — surfaced
     as-is to the frontend rather than pretending to succeed.
...
| `PAYSTACK_PREFERRED_BANK` | Partner bank for dedicated virtual accounts | auto: `test-bank` for a test key, `wema-bank` for a live key (see below) |
