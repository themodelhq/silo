...
from app.payments import split_amount_by_rules, verify_webhook_signature, default_preferred_bank
...
# ---- preferred_bank auto-detection (test vs live Paystack keys) ---------
# Paystack requires "test-bank" as the preferred_bank when creating a
# dedicated virtual account with a *test* secret key — the live-mode bank
# slugs ("wema-bank", etc.) are silently rejected with a test key, and this
# was a real, previously-undetected cause of DVA creation failing even with
# a correctly-configured, valid Paystack key.

def test_default_preferred_bank_is_test_bank_for_test_keys(monkeypatch):
    monkeypatch.delenv("PAYSTACK_PREFERRED_BANK", raising=False)
    monkeypatch.setenv("PAYSTACK_SECRET_KEY", "sk_test_abc123")
    assert default_preferred_bank() == "test-bank"


def test_default_preferred_bank_is_wema_bank_for_live_keys(monkeypatch):
    monkeypatch.delenv("PAYSTACK_PREFERRED_BANK", raising=False)
    monkeypatch.setenv("PAYSTACK_SECRET_KEY", "sk_live_abc123")
    assert default_preferred_bank() == "wema-bank"


def test_default_preferred_bank_respects_explicit_override(monkeypatch):
    monkeypatch.setenv("PAYSTACK_SECRET_KEY", "sk_test_abc123")
    monkeypatch.setenv("PAYSTACK_PREFERRED_BANK", "titan-paystack")
    assert default_preferred_bank() == "titan-paystack"
