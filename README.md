...
One screen is the exception: **Settings → Payment account** requests a real
Paystack dedicated virtual account from the backend (see `backend/README.md`
for what that needs server-side). Every other screen stays fully local
regardless of whether a backend is configured. This is the frontend's only
network-dependent feature, and it's covered by a real integration test —
`frontend/tests/payment_account_integration.test.js` loads the actual
`app.js` source into a sandboxed environment and drives it against a live
backend instance (register → login → wrong-password handling → account
creation error paths), rather than just asserting against a re-implementation
of the same logic. Run it with:
