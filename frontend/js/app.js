...
function openEnvelopeDetail(id) {
  ...
  document.getElementById("env-detail-body").innerHTML = `
    ...
    <div class="field mt-16">
      ...
      <select class="input" id="transfer-target">
        <option value="__account__" style="font-weight:700;">Account</option>
        ${otherEnvs.length ? `<optgroup label="Envelopes">${otherEnvs.map((e) => `<option value="${e.id}">${escapeHTML(e.name)}</option>`).join("")}</optgroup>` : ""}
      </select>
    </div>
    <div id="transfer-account-fields" style="display:none;">
      <div class="field">
        <label for="transfer-bank">Bank</label>
        <select class="input" id="transfer-bank"><option value="">Loading banks…</option></select>
      </div>
      <div class="field">
        <label for="transfer-account-number">Account number</label>
        <input class="input" id="transfer-account-number" placeholder="0123456789" maxlength="10" inputmode="numeric">
        <div id="transfer-account-resolved" class="text-secondary" style="font-size:12px;margin-top:6px;"></div>
      </div>
      <div class="field">
        <label for="transfer-reason">Reason (optional)</label>
        <input class="input" id="transfer-reason" placeholder="e.g. Rent payment">
      </div>
    </div>
    <div class="field">
      <label for="transfer-amount">Amount</label>
      <input class="input" id="transfer-amount" type="number" placeholder="Amount">
    </div>
  ...
  targetSelect.addEventListener("change", onTransferTargetChange);
  onTransferTargetChange(); // sets initial visibility (Account is the default selection)

  const accountNumberInput = document.getElementById("transfer-account-number");
  accountNumberInput.addEventListener("input", debounce(handleAccountNumberInput, 500));
  document.getElementById("transfer-bank").addEventListener("change", handleAccountNumberInput);
...
function onTransferTargetChange() {
  const isAccount = document.getElementById("transfer-target").value === "__account__";
  document.getElementById("transfer-account-fields").style.display = isAccount ? "block" : "none";
  if (isAccount) loadBanksIfNeeded();
}
...
async function loadBanksIfNeeded() {
  ...
  if (state.banksCache) {
    renderBankOptions(select, state.banksCache);
    return;
  }
  if (!state.user?.backend?.token) {
    select.innerHTML = `<option value="">Connect a Silo account first (Settings → Payment account)</option>`;
    return;
  }
  try {
    const banks = await backendFetch("/payments/banks", { headers: { Authorization: `Bearer ${state.user.backend.token}` } });
    state.banksCache = banks;
    renderBankOptions(select, banks);
  } catch (err) {
    select.innerHTML = `<option value="">Couldn't load banks — ${escapeHTML(err.message)}</option>`;
  }
}

function renderBankOptions(select, banks) {
  select.innerHTML = `<option value="">Select a bank</option>` + banks.map((b) => `<option value="${escapeHTML(b.code)}">${escapeHTML(b.name)}</option>`).join("");
}

async function handleAccountNumberInput() {
  ...
  const resolvedEl = document.getElementById("transfer-account-resolved");
  resolvedEl.dataset.accountName = "";
  ...
  if (!state.user?.backend?.token) {
    resolvedEl.textContent = "Connect a Silo account first (Settings → Payment account) to verify recipients.";
    return;
  }

  resolvedEl.textContent = "Verifying account…";
  try {
    const result = await backendFetch("/payments/resolve-account", {
      method: "POST", headers: { Authorization: `Bearer ${state.user.backend.token}` },
      body: JSON.stringify({ account_number: accountNumber, bank_code: bankCode }),
    });
    resolvedEl.textContent = `✓ ${result.account_name}`;
    resolvedEl.dataset.accountName = result.account_name;
  } catch (err) {
    resolvedEl.textContent = err.message;
  }
}
...
async function handleTransfer(fromId) {
  ...
  if (target === "__account__") {
    await handleAccountTransfer(fromId);
    return;
  }
...
/**
 * Sends money out of an envelope to an arbitrary bank account via Paystack
 * (backend: POST /payments/transfers). Requires a connected backend session
 * (see Settings → Payment account) since this moves real money, unlike the
 * rest of the app's fully-local envelope-to-envelope transfers.
 */
async function handleAccountTransfer(fromId) {
  const from = state.envelopes[fromId];
  const bankCode = document.getElementById("transfer-bank").value;
  const accountNumber = document.getElementById("transfer-account-number").value.trim();
  const resolvedEl = document.getElementById("transfer-account-resolved");
  const accountName = resolvedEl.dataset.accountName || "";
  const reason = document.getElementById("transfer-reason").value.trim();
  const amount = parseFloat(document.getElementById("transfer-amount").value);

  if (!state.user?.backend?.token) {
    toast("Connect your Silo account first — Settings → Payment account.", "danger");
    return;
  }
  if (from.locked) { toast(`${from.name} is locked.`, "danger"); return; }
  if (!bankCode) { toast("Choose a bank.", "danger"); return; }
  if (accountNumber.length < 10) { toast("Enter a valid account number.", "danger"); return; }
  if (!accountName) { toast("Waiting for the account to be verified — check the account number and bank.", "danger"); return; }
  if (!amount || amount <= 0) { toast("Enter a valid amount.", "danger"); return; }
  if (from.balance < amount) { toast(`Insufficient balance in ${from.name}.`, "danger"); return; }
  ...
  try {
    const transfer = await backendFetch("/payments/transfers", {
      method: "POST", headers: { Authorization: `Bearer ${state.user.backend.token}` },
      body: JSON.stringify({
        envelope_id: fromId, account_number: accountNumber, bank_code: bankCode,
        account_name: accountName, amount, reason: reason || undefined,
      }),
    });

    // Mirror the debit locally so the UI reflects it immediately — the
    // backend already debited its own copy of this envelope's balance.
    // If the transfer later fails, the backend refunds on its side; the
    // person will see the corrected balance next time this app syncs with
    // the backend (this build's envelope balances are otherwise fully
    // local/offline, so there's no live push from server to client here).
    from.balance = Math.round((from.balance - amount) * 100) / 100;
    await saveEnvelope(from);
    await saveTransaction({
      id: genId(), envelopeId: fromId, type: "expense", amount, category: from.name,
      merchant: accountName, note: `Transfer to ${accountNumber} (${transfer.status})`, occurredAt: new Date().toISOString(),
    });

    toast(
      transfer.status === "otp_required"
        ? "Transfer needs OTP confirmation in the Paystack dashboard before it completes."
        : `Sent ${fmt(amount)} to ${accountName}.`,
      transfer.status === "otp_required" ? "info" : "success"
    );
    closeModal("envelope-detail");
    renderEnvelopes(); renderDashboard();
  } catch (err) {
    // Same reasoning as requestPaymentAccount()'s catch block: show the
    // backend's actual detail on a 502, not a hardcoded guess at the cause.
...
async function applyPayslipEnvelopeSplit(p) {
  const positiveItems = p.lineItems.filter((li) => li.amount > 0);
  const rawTotal = positiveItems.reduce((sum, li) => sum + li.amount, 0);
  if (rawTotal <= 0) return Object.keys(state.envelopes).length;

  // netSalary is the actual money being distributed — every envelope's
  // *funded* balance must scale to it exactly, so Available Balance (the
  // sum of every envelope) always equals Net Salary, penny for penny. Each
  // envelope's *share* of that money comes from its proportion of the full
  // payslip breakdown (rawTotal), which is usually gross-level (basic +
  // every allowance) rather than net — dividing by net salary here would
  // make percentages that don't reflect real proportions and, funded
  // directly, wouldn't sum to net salary at all (the original bug).
  const netSalary = p.netSalary > 0 ? p.netSalary : rawTotal;

  const newRules = positiveItems.map((li, i) => ({
    id: genId(),
    envelopeName: li.name,
    type: PEEngine.AllocationType.PERCENTAGE,
    value: Math.round((li.amount / rawTotal) * 10000) / 10000, // this envelope's share of the payslip breakdown
  ...
  // Fund each envelope as exactly its share of net salary. The last
  // envelope absorbs whatever rounding remainder is left, so the total
  // funded is *exactly* net salary — never a cent off due to per-envelope
  // rounding — which is what makes Available Balance == Net Salary hold
  // precisely, not just approximately.
  let fundedSoFar = 0;
  for (let i = 0; i < newRules.length; i++) {
    const rule = newRules[i];
    const isLast = i === newRules.length - 1;
    const amount = isLast ? Math.round((netSalary - fundedSoFar) * 100) / 100 : Math.round(rule.value * netSalary * 100) / 100;
    fundedSoFar = Math.round((fundedSoFar + amount) * 100) / 100;
...
// The example domain shipped in js/config.js — not a real deployed backend.
// If this is still what's configured, the person hasn't set their own
// backend URL yet; say so plainly instead of a generic network-error message.
const UNCONFIGURED_PLACEHOLDER_HOSTS = ["silo.onrender.com", "silo-api.onrender.com", "payenvelope.onrender.com"];

async function backendFetch(path, options = {}) {
  const base = apiBaseUrl();
  if (!base) throw new Error("The backend isn't configured yet — set SILO_CONFIG.API_BASE_URL in js/config.js to your deployed backend's URL.");

  let isPlaceholder = false;
  try { isPlaceholder = UNCONFIGURED_PLACEHOLDER_HOSTS.includes(new URL(base).hostname); } catch { /* malformed URL, let fetch() surface it below */ }
  ...
  try {
    ...
    if (isPlaceholder) {
      throw new Error(`${base} is still the example backend URL from js/config.js — it isn't a real deployed backend. Deploy the backend (see backend/README.md) and set SILO_CONFIG.API_BASE_URL to its real URL.`);
    }
...
async function requestPaymentAccount() {
  ...
  try {
    ...
    // 502 means the backend's Paystack call itself failed — err.message is
    // the backend's actual detail (e.g. an invalid key, an unverified
    // Paystack account, a rejected phone/bank format, or a real "missing
    // key" case), never a hardcoded guess. Masking this with one fixed
    // message made real misconfigurations impossible to diagnose from the
    // UI — always show what the server actually said.
    toast(err.message, "danger");
  } finally {
