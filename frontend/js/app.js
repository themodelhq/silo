...
const PEParser = window.PayEnvelopeParser;
const PEEngine = window.PayEnvelopeEngine;
const genId = window.PayEnvelopeEngine.uuid;
const Storage = window.PayEnvelopeStorage;
...
async function confirmAllocatePayslip() {
  ...
  // seed engine with existing envelopes so ids are preserved where names match
  ...
  toast(`${fmt(p.netSalary, p.currency)} allocated across ${Object.keys(engine.envelopes).length} envelopes.`, "success");
...
function handleFileSelected(file) {
  ...
  if (file.type === "text/plain" || file.name.endsWith(".txt")) {
    ...
    toast("For PDFs/images, please paste the payslip text below — PayEnvelope parses text deterministically without cloud OCR.", "info");
  }
}
...
function exportReportCSV() {
  ...
  a.href = url; a.download = "payenvelope-transactions.csv"; a.click();
...
function renderRulesModal() {
  ...
  document.getElementById("save-rules-btn").addEventListener("click", async () => {
    ...
    try {
      window.PayEnvelopeEngine.validateRuleSet(state.rules);
...
async function handleRegister(e) {
  ...
  state.user = { id: genId(), fullName, email, country, settings: { currency: COUNTRY_TO_CURRENCY[country], country, theme: "dark", highContrast: false } };
...
function wireEvents() {
  ...
  document.getElementById("export-data-btn").addEventListener("click", async () => {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = "payenvelope-export.json"; a.click();
    URL.revokeObjectURL(url);
    toast("Data exported.", "success");
  });
  document.getElementById("delete-account-btn").addEventListener("click", async () => {
    if (!confirm("This deletes all local PayEnvelope data on this device. Continue?")) return;
