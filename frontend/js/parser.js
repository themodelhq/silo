...
const FIELD_PATTERNS = {
  ...
  basicSalary: new RegExp(`(?:basic|base)\\s*(?:salary|pay|allowance)?\\s*[:\\-=]?\\s*${NUM}`, "i"),
...
// Keywords that mark a line as identity info, a statutory figure, or a
// document total — excluded so they don't become nonsense "envelopes"
// (case-insensitive substring match on the label). Deliberately narrow:
// every other line item — including earnings like "Basic Allowance" or
// "Housing Allowance" — becomes its own envelope with its exact payslip
// name, even if a dedicated structured field (basicSalary, etc.) also
// captured it for the gross/net summary. The envelope split should mirror
// every allocation on the payslip, not a filtered subset of them.
const LINE_ITEM_EXCLUDE_KEYWORDS = [
  ...
  "total deduction", "total earning", "total income", "total pay",
];

// Statutory-deduction words. Excluded on their own (e.g. a bare "Pension:
// 20,000" deduction line) but NOT when the label also contains "allowance"
// or "income" — real payslips occasionally name an earnings item something
// like "Other Income Pension Allowance", which is a budget category, not
// the statutory pension deduction, even though it contains "pension".
const LINE_ITEM_EXCLUDE_STATUTORY_KEYWORDS = ["tax", "paye", "pension", "ssnit", "nssf", "uif", "nhf"];
...
function extractLineItems(text) {
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    const isEarningsStyle = normalized.includes("allowance") || normalized.includes("income");
    if (!isEarningsStyle && LINE_ITEM_EXCLUDE_STATUTORY_KEYWORDS.some((kw) => normalized.includes(kw))) continue;
